...
import pandas as pd
from MOMENTR import *  # This includes the update_data function

# Load initial data from CSV files into session state
if "all_m_ret" not in st.session_state:
    st.session_state.all_m_ret = pd.read_csv(
        r"C:\Users\User\Desktop\GLADIO\MOMENTUM APP\all_monthly_returns.csv", index_col=0, parse_dates=True
    )
    st.session_state.all_w_ret = pd.read_csv(
        r"C:\Users\User\Desktop\GLADIO\MOMENTUM APP\all_weekly_returns.csv", index_col=0, parse_dates=True
    )
...
def load_benchmark_and_rf(benchmark_ticker, risk_free_ticker):
    benchmark = yf.download(benchmark_ticker, start='2010-01-01')[['Adj Close']]
    benchmark.columns = ["Benchmark"]
    risk_free = yf.download(risk_free_ticker, start='2010-01-01')[['Adj Close']]
...
if st.sidebar.button("Update Data"):
    st.sidebar.write("Updating data... this may take a while.")
    all_m_ret, all_w_ret = update_data()
    st.session_state.all_m_ret = all_m_ret
    st.session_state.all_w_ret = all_w_ret
